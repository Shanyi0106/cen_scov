...
Rcpp::sourceCpp("./covglasso_ada.cpp")
source("./utils.R")

covglasso_ada <- function(data = NULL,
                          S = NULL,
                          n = NULL,
                          lambda = NULL,
                          lam = NULL,
                          ...
                          )
{
  ...
  V <- ncol(S)
  varnames <- colnames(S)
  crit <- match.arg( crit, choices = eval(formals(covglasso_ada)$crit) )
  ...
  pen__type <- 0
  if(pen_type == "L1")  {pen__type <- 1}
  if(pen_type == "SCAD"){pen__type <- 2}
  if(pen_type == "MCP") {pen__type <- 3}

  fit <- covglassopath_bic_ada(S, lambda, start = start, n = n, L = LL,
                           tolout = ctrl$tol.out, tolin = ctrl$tol.in,
                           iterout = ctrl$iter.out, iterin = ctrl$iter.in,
                           lam = lam, pen_type = pen__type, ga = ga)
